...
const parseNavigationItems = function(tokens, m: Manual) {
  const anchors: ManualAnchor[] = []; // This is the return value.
  ...
  for (const token of tokens) {
    switch (token.type) {
      case 'heading': {
        ...
        const icon = m.baseUrl + extracted.icon;
  ...
  m.appName = appInfo.name;
  m.appIcon = appInfo.icon;
  m.headers = headers;
  m.anchors = anchors;
};
...
const getHTMLRenderer = function(baseUrl: string) {
  const renderer = new marked.Renderer();
  ...
  renderer.image = function(href, title, text) {
    ...
    let out = `<img src="${baseUrl}${href}" alt="${text}"`;
    if (title) {
      out += ` title="${title}"`;
    }
...
const processMarkdown = function(src, m: Manual) {
  // Lex
  ...
  // Extract Headers
  parseNavigationItems(tokens, m);
  console.log('parsed: ', m);

  // Pass tokens to HTML renderer
  m.html = marked(src, {
    renderer: getHTMLRenderer(m.baseUrl),
  });
};
...
export class Manual {
  ...
  static parseMarkdown(src: string, baseUrl: string): Manual {
    const m = new Manual();
    m.baseUrl = baseUrl;
    processMarkdown(src, m);
    console.log(m.html);
    return m;
  }
