...
統信操作系統（簡稱「統信UOS」）是一款美觀易用、安全可靠的國產桌面操作系統。統信UOS預裝了檔案管理員、系統監視器等一系列原生應用。它既能讓您體驗到豐富多彩的娛樂生活，也可以滿足您的日常工作需要。隨着功能的不斷升級和完善，統信操作系統已成為國內最受歡迎的桌面操作系統之一。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
> ![tips](../common/tips.svg) 竅門：您還可以在圖片查看器中設置您喜歡的圖片為桌面壁紙。
...
1. 右鍵單擊任務欄。
2. 在 **外掛** 子菜單中勾選或取消勾選 **回收站、系統監視器、電源、顯示桌面、螢幕鍵盤、通知中心、時間**，可以設置這些外掛在任務欄上的顯示和隱藏效果。
...
1. 在控制中心首頁，單擊![account_normal](../common/account_normal.svg)。
2. 單擊當前帳號。
3. 打開 **自動登錄** 開關，在弹出的授权对话框中输入当前帐户密码并确定后，開啟自動登錄功能。
...
1. 在控制中心首頁，單擊 ![display_normal](../common/display_normal.svg)。
2. 在方向區域的下拉列表中，選擇一個合適的方向參數，改變螢幕方向。
3. 單擊 **保存**。
...
藍牙能夠實現短距離的無線通信。通過藍牙無需網絡或連接線就可以與附近的其他藍牙設備連接。常見的藍牙設備包括藍牙鍵盤、藍牙鼠標、藍牙耳機、藍牙音響等。

![0|bluetooth](fig/e_bluetooth.png)

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

> ![notes](../common/notes.svg) 說明：有些電腦沒有配備藍牙，您可以購買藍牙適配器，接入電腦中使用。
...
4. 更新完成後，在彈出的對話框中，根據實際需要執行 **重啟** 、**關機** 或 **取消** 操作。
...
> ![attention](../common/attention.svg) 注意：只有在試用期或被啟用成功的系統，才能正常使用统信UOS的所有功能，否則會有部分功能受到限制。
...
每台設備僅有一次試用期啟用機會（默認開機自動啟用），從啟用當天算起，有效期為180天。試用期間可使用系統全部功能。試用期間可使用系統全部功能，也可隨時通過序列號或啟用文件永久啟用系統。
...
系統成功啟用後，可查看啟用狀態、試用到期時間等訊息，也可以選擇通過輸入序列號、導入啟用文件來啟用系統。
...
![0|backup-recovery](fig/e_backupRestore.png)
