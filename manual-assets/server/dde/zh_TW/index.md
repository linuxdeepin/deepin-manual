...
桌面是您登入後看到的主螢幕區域。在桌面上，您可以新增文件/資料夾、排列文件、開啓終端機、設定桌布等，還可以透過 [設定捷徑](#設定捷徑) 向桌面添加應用程式的捷徑。
...
對系統電源進行一些設定，可以設定**顯示器關閉時間**、**自動鎖定螢幕時間**等。
...
1. 在控制中心首頁，單擊 ![power_normal](../common/power_normal.svg)。
2. 單擊 **使用外接電源**。
3. 選擇電源按鈕 **關機**、**關閉顯示器** 或 **無任何操作**，更改電源設定。
...
每台裝置僅有一次試用期啟用機會（預設開機自動啟用），從啟用當天算起，有效期為180天。試用期完成後，您可以透過購買序號等方式正式啟用系統。
