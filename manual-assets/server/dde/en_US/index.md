...
When you log into the system for the first time, a welcome program will automatically start.  Watch the introduction video to get new features, customize your desktop and icon theme, and know more about UnionTech OS.
...
> ![tips](icon/tips.svg) Tips: You can set your favorite picture as wallpaper in Image Viewer.
...
> ![tips](icon/tips.svg) Tips: In Efficient Mode, you can click the right side of Dock to show the desktop. Move the cursor to the running application in the Dock and you will see its preview window.
...
>![tips](icon/tips.svg) Tips: You can view newly installed applications in Launcher. The newly-installed ones are followed by a blue dot.
...
You can manage the system settings in Control Center, including account management, network settings, date and time, personalization, display settings, system update, etc. After entering the desktop environment, click ![controlcenter](icon/controlcenter.svg) to open Control Center.
...
You can set theme, accent color, font, change the appearance of the desktop and windows to your favorite style. You can also set the icon theme, cursor theme, and font.
...
1. On the homepage of Control Center, click ![power_normal](icon/power_normal.svg).
2. Click **Plugged In**.
3. You can select **Shut down**, **Turn off the monitor**, or **Do nothing** from the drop-down list after **When pressing the power button**.
...
Mouse is a common computer input device. Using the mouse, you can make the operation easier and faster.  In the Control Center, users can set the mouse so that the experience of using the device is more in line with their own habits.
...
> ![notes](icon/notes.svg) Notes: If "Left Hand" is enabled, left-click and right-click of the mouse exchange.
...
> ![notes](icon/notes.svg) Notes: You can click the button to pause and resume the downloading process. After updating finishes, a dialogue box pops up to remind you to **Reboot** or **Shut down** your computer to make the updates come into effect or **Cancel** operations.
...
The inactive status includes being unactivated and being unactivated after expiration.
...
> ![attention](icon/attention.svg) Attention: Only systems in the trial period or the ones activated successfully can all the functions of UnionTech OS be used normally, otherwise some functions will be limited. 
...
It is available to view the information such as expiration date and so on. The system could also be activated by serial number and license files.
...
![0|activationcode1](jpg/activationcode1.png)
...
1. It will jump to the off-line activation interface when abnormal network connection is detected by the system in the process of activating the system by inputing the serial number or  importing the authorization file.
   ...
   ![0|activateoffline1](jpg/activateoffline1.png)
...
2. Scan the QR code with your mobile phone  and the current device ID as well as activation code are displayed in the pop-up activation interface.
   ...
   ![0|activateoffline2](jpg/activateoffline2.png) 
...
![0|activationcode3](jpg/activationcode3.png)
...
2. Select **Backup and Restore** > **Backup** to set the backup type and save the path of files backed up.
   ...
   > ![notes](icon/notes.svg) Notes:  **Full Backup** cannot backed up files on its own disk, but can only back up files in other storage media.
...
> ![notes](icon/notes.svg) Notes: arm architecture does not support startup menu setting for now; mips architecture does not support modifying boot menu background for now.
...
<div class="version-info"><span>Update Date: 2021-08-18</span><span> Version: 2021.07</span></div>