...
Click ![icon_menu](icon/icon_menu.svg) > **Mode**.

- Select **Standard Mode** to execute the four fundamental operations of arithmetic(addition, subtraction, multiplication and division).
- Select **Scientific Mode** to calculate high-level operations such as function, exponent, root, and so on. 
...
| -------------------------- | :-------------------------------------------------- | ------------------------------------------------------------ |
| 0~9                        | Number Key                                          | Basic Arabic numeral                                         |
| MC                         | Clear Key                                           | Clear all memories                                           |
| MR                         | Storage Key                                         | Memory recall                                                |
| M+                         | Storage Key                                         | Memory add; click ![m+](icon/M+.svg) to add the current number accumulatively to the memory and interrupt digital input. |
| M-                         | Storage Key                                         | Memory subtract; click ![m-](icon/M-.svg) to subtract the current number from the memory and interrupt digital input. |
| MS                         | Storage Key                                         | Memory store；click ![ms](icon/MS.svg) to add the numeric value in the input box to the memory list. |
| ![M](icon/M.svg)           | Storage Key                                         | Click ![M](icon/M.svg)to expand the memory list; click it again to fold the memory list. The memory will be cleared up when Calculator is closed. |
...
| ![icon](jpg/deg.png)/![icon](jpg/grad.png)/![icon](jpg/rad.png) | Metric conversions   | Click this button to calculate the angle, gradient and radian respectively. |
...
| ![icon](jpg/square_root.png), ![icon](jpg/cube_root.png), ![icon](jpg/y_root.png) | Power function | Click 2<sup>nd</sup> to switch to the second function interface. Click them to calculate the square root, cube root and y-th root of X respectively. |
...
- Fault-tolerance processing of percent sign: Input percent sign (%)  to trigger percent sign;
...
<div class="version-info"><span>Update Date: 2021-08-18</span><span> Version: 5.7</span></div>