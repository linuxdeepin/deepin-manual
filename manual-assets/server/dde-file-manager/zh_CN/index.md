...
- 在任务栏右键单击 ![deepin_file_manager](icon/deepin_file_manager.svg)，选择 **关闭所有** 来退出文件管理器。
- 在文件管理器界面，单击 ![icon_menu](icon/icon_menu.svg)，选择 **退出** 来退出文件管理器。

### 查看快捷键

在文件管理器界面上，按下 **Ctrl + Shift + ?** 组合键来查看快捷键，熟练地使用快捷键，将大大提升您的操作效率。
...
| 标号 | 名称          | 描述                                                         |
| ---- | ------------- | ------------------------------------------------------------ |
| 1    | 导航栏        | 单击导航栏图标，快速访问本地文件、磁盘、网络邻居、书签、标记等。 |
| 2    | 地址栏        | 通过地址栏，可以快速切换访问历史、在上下级目录间切换、搜索等。 |
...
| 5    | 菜单栏        | 通过主菜单，您可以新建窗口、切换窗口主题、设置共享密码、设置文件管理器、查看帮助文档等。 |
...
> ![tips](icon/tips.svg)窍门：选择相同类型的多个文件，右键单击并选择“打开方式”选项，可同时打开多个文件。
...
> ![attention](icon/attention.svg)注意：外接设备中删除文件会将文件彻底删除，无法从回收站找回。
...
>![attention](icon/attention.svg)注意：如果上面的这些操作是连续的，就可以一直撤销；如果操作中有覆盖某个同名文件和彻底删除文件，则撤销只能返回到这一步。
...
单击搜索图标，或者在文件路径上右键选择“编辑地址”，地址栏会切换为输入框状态。在输入框外单击时，地址栏会自动恢复到面包屑状态。

![0|addressbar](jpg/addressbar2.png)

- 输入框带有智能识别功能，您可以输入关键字或者访问地址，系统会自动识别并进行搜索或访问。
...
> ![notes](icon/notes.svg)说明：若想隐藏“最近使用”，可在设置 > 基本设置 > 隐藏文件，取消勾选“显示最近使用文件”。若要隐藏某一个文件的访问记录，右键单击该文件选择 **移除**，该操作不会删除文件。
...
1. 在文件管理器界面上，右键单击文件夹并选择 **共享文件夹**。
3. 在文件夹的属性窗口，勾选 **共享此文件夹**。
4. 根据需要设置 **共享名**、**权限** 、**匿名访问** 后关闭窗口。
5. 在文件管理器界面上，单击 ![icon_menu](icon/icon_menu.svg)> **设置共享密码**。
7. 输入共享密码，单击 **确定**。
...
2. 输入用户名密码或者匿名访问。
 ...
 - 加密的网络文件会弹出登录框，输入用户名和密码之后才能访问。如果在用户名密码提示框中勾选 **记住密码**，再次访问不再需要密码。

      - 用户名：共享者登录系统的用户名。
      - 密码：共享文件时设置的共享密码。
3. 单击 **连接**。

![1|visitshare](jpg/visitshare.png)
...
![0|open-terminal](jpg/openterminal.png)
...
<div class="version-info"><span>文档更新时间: 2021-01-22</span><span> 版本: 5.2</span></div>