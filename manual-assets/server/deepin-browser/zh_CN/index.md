...
2. 上下滚动鼠标滚轮浏览或通过搜索，找到浏览器图标![deepin-browser-24](icon/deepin-browser-24.svg) ，单击运行。
...
1. 在浏览器窗口，打开一个网页。
2. 在键盘上按下 **Ctrl** +**F**，在窗口的右上角出现的栏中输入搜索字词，按 **Enter** 键搜索网页。
4. 系统会以黄色突出显示匹配内容。您可以使用滚动条上的黄色标记查看所有匹配内容在网页中的位置。

#### 搜索内容

1. 在浏览器中打开一个网页，鼠标左键选中某个字词、短语。
2. 单击右键并选择搜索选项，即可使用您的默认搜索引擎开始搜索相关网页。
...
4. 下载完毕后，单击 ![download](icon/download.svg) 查看下载内容，也可以打开下载文件所在的目录。
...
单击下载按钮 ![download](icon/download.svg) ，进入下载内容界面，可以选择暂停或取消下载的文件。
...
1. 在浏览器窗口，单击  ![icon_menu](icon/icon_menu.svg)  > **设置**。
2. 选择 **下载设置** 并单击 **更改**... ，可以设置下载文件的默认存放路径。

> ![tips](icon/tips.svg)窍门：如果您要在每次下载时选择具体的位置，请选中“下载前询问每个文件的保存位置”复选框。

#### 查看下载记录

在浏览器窗口，单击 下载按钮 ![download](icon/download.svg) ，可以查看过往的下载记录、查询下载文件。
...
3. 如果勾选 **退出浏览器时自动清除选中项**，将弹出 **清除上网痕迹** 窗口，单击 **立即清除**。
4. 单击 **清除**。
...
如果您使用的是计算机，并且发现主页或启动页并不是您自己设置的网页，那么您的计算机上可能存在恶意软件。

如果使用的是单位或学校的浏览器，网络管理员可能会为您选择启动页或主页。如果是这样，那么您无法进行更改。如需获取更多帮助，请咨询您的管理员。
...
1. 在浏览器窗口，单击  ![icon_menu](icon/icon_menu.svg)  > **设置 **。
2. 在“自动填充”区域，勾选 **密码**，并单击"密码"旁边的更多图标 >，进入 **密码** 详情窗口。
3. 可查看已保存密码的网站及删除密码。
