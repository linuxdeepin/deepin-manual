# 文件管理器|../common/dde-file-manager.svg|
...
> ![tips](icon/tips.svg)窍门：您也可以使用快捷键 **Super + E** 启动文件管理器。
...
>![notes](icon/notes.svg)说明：如果文件有标记，则复制或剪切该文件产生的文件仍然具有同样的标记。
...
![0|open_terminal](jpg/openterminal.png)
...
统信UOS助手是一款实现手机与电脑间跨端协作、高效传输的实用工具，只需要将手机与PC端连接同一无线网络，便可以轻松传输图片、视频、文件等，同时也支持在线编辑手机文件，手机或PC端实时同步。

### 安装统信UOS助手

1. 单击系统托盘中的图标 ![icon](icon/phone.svg) 或打开文件管理器，单击左侧导航栏 **我的手机**。
2. 单击 **安装统信UOS助手**，弹出二维码窗口。
   ![phone](jpg/myphone.png)

3. 打开手机，通过微信或浏览器扫描弹窗中的二维码，下载并安装统信UOS助手；您也可以单击弹窗中的按钮 **下载APK安装包**，将安装包下载到本地，再导入至手机中进行安装。

   >![icon](icon/notes.svg)说明：仅支持安卓客户端。
...
3. 打开手机端统信UOS助手，单击 **扫码连接**，扫描PC端窗口中的二维码，连接成功后便可以进行文件传输的操作。

   ![icon](jpg/connect_to_pc.png)

>![icon](icon/notes.svg)说明：您可以单击手机端按钮 ![icon](icon/exit.svg) 断开连接，或右键单击系统托盘中的图标 ![icon](icon/phone.svg)，选择您的手机，在弹出的窗口中单击 **断开连接**。

### 传输文件

统信UOS助手支持发送手机图片、视频、文件等到PC端，也支持接收PC端文件，并同步保存至手机文件夹中。

#### 手机端至PC端

打开手机端统信UOS助手，单击 **文件传输**。在文件传输界面，可以进行以下操作：

- 发送文件到PC端：传输成功后桌面弹出提示窗口，单击窗口中 **查看所在位置**，定位该文件所在的存储位置。传输文件默认保存至 **文件管理器 > 下载 > xxx（手机名称）** 中。
   <table class="block1">
    <caption></caption>
    <tbody>
        <tr>
            <td><img src="icon/pic.svg" alt/></td>
            <td>传输手机中的图片和视频，支持拼接多张图片。</td>
        </tr>
        <tr>
            <td><img src="icon/camera.svg" alt/></td>
            <td>拍摄照片或录制视频发送到PC端，并同步保存至手机中。  </td>
        </tr>
    	<tr>
     ...
     ![icon](jpg/transfer.png)
   
- 发送文字到PC端：在手机端文本输入框中输入文字并发送，桌面弹出提示信息，该段文字被复制至剪切板中。

- 分享文件：打开已传输的文件，单击右下角按钮 ![icon](jpg/share_icon.png) ，将文件分享到第三方应用。

- 查看最近文件、清空记录：单击右上角按钮 ![icon](icon/operation_button.svg)  ，查看历史记录或清除全部记录。

>![icon](icon/notes.svg)说明：您还可以在PC端我的手机界面，选择某一个文件，右键单击选择 **保存到电脑**。

#### PC端至手机端

- 在PC端，右键单击文件或文件压缩包，选择 **发送到手机**。
- 在PC端，拖拽文件到文件管理器左侧导航栏 **我的手机** 上。
...
> ![notes](icon/notes.svg)说明：如果磁盘或磁盘中的文件夹已加密，请输入密码后查看文件。
...
<div class="version-info"><span>文档更新时间: 2021-06-23</span><span> 版本: 5.2</span></div>