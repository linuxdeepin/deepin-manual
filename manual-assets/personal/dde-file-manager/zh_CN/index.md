...
统信UOS助手是一款实现手机与电脑间跨端协作、高效传输的实用工具，只需要将手机与PC端连接同一Wi-Fi，便可以轻松传输图片、视频、文件等。

### 安装统信UOS助手

1. 单击系统托盘中的图标 ![icon](icon/phone.svg) 或打开文件管理器，单击左侧导航栏 **我的手机**。
2. 单击 **下载统信UOS助手**，弹出二维码窗口。
...
- 发送文件到PC端：传输成功后桌面弹出提示窗口，单击窗口中 **查看所在位置**，定位该文件所在的存储位置。传输文件默认保存至 **文件管理器 > 下载 > UOS_DOWNLOADS** 中。
   <table class="block1">
    <caption></caption>
    <tbody>
        <tr>
...
- 分享文件：打开已传输的文件，单击右下角按钮 ![icon](jpg/share_icon.png) ，可以选择将文件分享到第三方应用。
- 查看最近文件、清空记录：单击右上角按钮 ![icon](icon/operation_button.svg)  ，可以查看历史记录或清除全部记录。
...
<div class="version-info"><span>文档更新时间: 2021-04-29</span><span> 版本: 5.2</span></div>