...
統信作業系統（簡稱「統信UOS」）是一款美觀易用、安全可靠的國產桌面作業系統。統信UOS預裝了檔案管理器、應用商店、看圖、影院等一系列原生應用程式。它既能讓您體驗到豐富多彩的娛樂生活，也可以滿足您的日常工作需要。隨著功能的不斷升級和完善，統信作業系統已成為中國最受歡迎的桌面作業系統之一。
...
桌面是您登入後看到的主螢幕區域。在桌面上，您可以新增文件/資料夾、排列文件、開啓終端機、設定桌布和螢幕保護程式等，還可以透過 [建立桌面捷徑](#設定捷徑) 向桌面添加應用程式的捷徑。
...
| ![browser-cn](../common/browser-cn.svg)           | 瀏覽器 - 點擊打開網頁。             | ![deepin-appstore](../common/deepin-appstore.svg)        | 應用商店 - 搜尋安裝應用軟體。                         |
| ![ deepin-album](../common/deepin-album.svg)                 | 相冊 - 匯入並管理照片。             | ![deepin-music](../common/deepin-music.svg)              | 音樂 - 播放本機音樂。                               |
| ![deepin-contacts](../common/deepin-contacts.svg)            | 聯系人 - 好友通訊，視訊會議。     | ![dde-calendar](../common/dde-calendar.svg)              | 日曆 - 查看日期、建立日程。                         |
...
| ![shutdown](../common/system-shutdown.svg)                   | 電源 - 點擊進入關機介面。            | ![trash](../common/trash_icon.svg)                       | 回收桶。                                             |
...
透過桌面智慧助手可以調節系統音量、螢幕亮度、還可以開啟/關閉電源節能模式、WiFi或藍牙。
...
> ![tips](../common/tips.svg)竅門：選中文字按下 **Ctrl** + **AIt** + **P** 可以進行語音播報；連接錄音裝置後，輸入框內按下 **Ctrl** + **AIt** + **O** 進入聽寫模式，把語音輸入轉換為文字。
...
1.  選中文字， 在鍵盤上按下快捷鍵 **Ctrl** + **AIt** + **P** 或在右鍵選單中選擇 **語音朗讀** ，單擊後喚起朗讀。
...
選中文字， 在鍵盤上按下快捷鍵 **Ctrl** + **AIt** + **U** 或在右鍵選單中選擇 **翻譯**， 彈出翻譯浮窗。 
...
1. 在控制中心首頁，單擊 ![display_normal](../common/display_normal.svg)。
2. 單擊 ![rotate](../common/rotate.svg) 。
3. 每單擊一下滑鼠左鍵，螢幕逆時針旋轉90°。
4. 要還原為之前的螢幕方向，單擊滑鼠右鍵退出；要使用目前螢幕方向，請按下組合鍵 **Ctrl** + **S** 儲存。
...
> ![notes](../common/notes.svg)說明：當多顯示器模式為延伸畫面模式時，僅主顯示器支援桌面圖示顯示、操作右鍵選單等功能，而副顯示器不支援。
...
2. 單擊 **多顯示器模式** > **自訂**。
3. 單擊 **辨識螢幕**，查看螢幕名稱。
4. 選擇 **合併** 或 **分割**，然後對多個螢幕進行設定，如主螢幕、解析度、刷新率、旋轉螢幕等。
5. 單擊 **儲存**。

> ![notes](../common/notes.svg)說明：合併即同步顯示畫面模式，分割即延伸畫面模式。
...
為了防止他人掃描到自己的WiFi，進而破解WiFi密碼連接您的網路，您可以在路由器設定介面隱藏無線網路。設定後，您需要手動連接隱藏網路才能上網。
...
無線熱點將電腦連接的網路訊號轉換為WiFi熱點，以供一定距離內的其他裝置無線上網。要開啟無線熱點，電腦必須連接到網路並裝有無線網卡。
...
前提條件：發送端（統信UOS PC）藍牙裝置與接收端（非統信UOS PC/手機/平板）藍牙裝置已配對並連接成功。


1. 在控制中心首頁，單擊 ![bluetooth_normal](../common/bluetooth_normal.svg)。
2. 開啟 **藍牙**，系統將自動掃描附近的藍牙裝置。
3. 進入 **我的裝置** 選擇已配對連接的藍牙裝置，單擊進入詳情，單擊 **發送文件** 按鈕（pc/手機/平板藍牙裝置類型才具有該按鈕），彈出選擇發送文件對話框。
4. 在彈出的選擇接收此文件的藍牙裝置視窗中，點選藍牙裝置名稱，單擊 **下一步**，發送傳輸文件請求。

   - 如果接收端藍牙裝置同意接收，則發送端開始傳輸文件，並顯示發送文件視窗；文件傳輸完成後，接收端會彈出提示信息，使用者可以查看文件內容。
   - 如果接收端為如下情況，發送端部分文件傳輸失敗，會顯示在傳輸失敗列表。
     - 接收端藍牙裝置選擇拒絕，則結束傳輸，顯示傳輸失敗。
     - 接收端藍牙裝置超時未操作或超出傳輸範圍，則顯示傳輸失敗（藍牙設備連接失敗）。
...
> ![notes](../common/notes.svg)說明：如果使用的是桌電，將不會看到節能設定和 **使用電池** 的選項。

#### 開啟節能模式

為降低筆記本的耗電量，您可以打開節能模式，降低螢幕亮度。
...
3. 打開 **節能模式**。
   - 打開 **使用電池時自動開啟** 節能模式，拔掉電源則直接進入節能模式，插上電源後自動退出節能模式。
   - 拖動亮度條滑塊，調節亮度。
...
使用者可以通過拖拽低電量滑塊，調節低電量的閾值；也可以通過拖拽自動待機電量滑塊，調節待機電量的閾值。
...
> ![tips](../common/tips.svg)竅門：您也可以選擇一組或多組快捷鍵，按順序切換已添加的鍵盤佈局。選擇 **套用至**，讓切換後的鍵盤佈局應用於整個系統或目前應用程式。
...
1. 執行授權管理工具有兩種方式：
   ...
   - 打開 **控制中心**，選擇 **系統資訊**，單擊 **關於此電腦**。可查看版本授權，單擊 **啟動** 進入頁面。

2. 未啟動狀態分為兩種：未啟動和過期未啟動。

   - 未啟動：介面包括產品名稱、產品版本和授權狀態訊息。

   - 過期未啟動：在截止時間內未啟動系統，介面包括產品名稱、產品版本、授權狀態、授權物件和截止日期訊息。

3. 可選擇啟動方式啟動系統。

> ![notes](../common/notes.svg)說明：如果系統未啟動，授權管理圖示 ![authorize1](../common/authorize1.svg) 一直常駐在右下角託盤區，開機即顯示。

#### 啟動方式

授權管理啟動方式分線上啟動和離線啟動，而線上啟動方式包含試用期啟動、輸入序號、匯入啟動文件。

> ![attention](../common/attention.svg)注意：只有在試用期或被啟動成功的系統，才能正常使用統信UOS的所有功能，否則會有部分功能受到限制。
...
在線/離線場景下都是開機自動激活，每台裝置僅有一次試用期啟動機會，從啟動當天算起，有效期為90天。試用期間可使用系統全部功能。
...
3. 單擊 **立即啟動**，會彈出二次確認彈框，單擊 **確定**，彈出試用期啟動成功介面，任務欄圖示變為 ![authorize2](../common/authorize2.svg)。
4. 在此介面，可查看試用到期時間等訊息，也可以選擇永久啟動系統。
...
2. 單擊 **輸入序號**，輸入或複製正確的序號，單擊 **立即啟動**。
   > ![notes](../common/notes.svg)說明：統信UOS既支援16位的序號，也支援25位的序號。

   ![0|activation_code1](fig/activation_code1.png)

3. 會彈出二次確認彈框，單擊 **確定**，彈出啟動成功介面。

匯入啟動文件

1. 單擊右下角託盤區上的 ![authorize1](../common/authorize1.svg) 進入介面。

2. 單擊 **匯入啟動文件**，彈出檔案管理器，選擇.key格式的啟動文件。

![0|file_activation2](fig/activation_via_file2.png)

3. 選擇正確的文件，單擊 **立即啟動**。會彈出二次確認彈框，單擊 **確定**，彈出啟動成功介面。

**離線啟動**

離線啟動的前提是系統網路連接異常。

1. 選擇輸入序號或匯入授權文件啟動時，系統檢測網路連接異常，則跳轉到離線啟動介面。

2. 離線啟動介面顯示二維碼、序號、機器ID及離線啟動碼輸入框。

![0|activate_offline1](fig/activation_offline1.png)

3. 手機掃描二維碼，手機端彈出啟動介面，介面顯示目前機器ID及序號。

   ![0|activate_offline2](fig/activate_offline2.png)

4. 單擊 **立即啟動**，會彈出二次確認彈框，單擊 **確定**。

5. 跳轉到啟動成功介面，顯示機器ID、序號和離線啟動碼。

   ![0|activate_offline3](fig/activate_offline3.png)

6. 手動將手機端的離線啟動碼輸入電腦端的離線啟動碼輸入框。
...
1. 啟動成功介面顯示產品名稱、產品版本和啟動方式等訊息。

 - 產品名稱：作業系統的名稱。
 - 產品版本：作業系統的版本。
 - 授權狀態：授權狀態分為未授權、已過期、已授權，啟動成功顯示已授權。
 - 授權物件：被授權的使用者名稱。
 - 啟動方式：啟動方式分為序號、啟動文件及離線啟動碼，根據實際活化方式體現。
 - 到期時間：超過了截止時間，系統會提醒授權即將到期。

 ![0|activation_code3](fig/activation_code3.png)
2. 系統啟動成功後，右下角託盤區不再顯示授權管理圖示，可透過控制中心查看授權管理工具。

#### 啟動失敗

若啟動文件已使用或已失效，則會彈出啟動失敗介面。

#### 啟動伺服器設定

如果目前啟動伺服器崩潰了，透過授權管理工具，單擊 ![menu](../common/icon_menu.svg) 主選單中的設定，可以自訂新的啟動伺服器。
...
> ![attention](../common/attention.svg)注意：全盤備份無法備份在自己本身的磁碟裡，只能備份在其它儲存介質中；系統備份設定了文件預設存放位置，當存放空間不足時，使用者可以手動更改備份文件的存放路徑。

3. 單擊 **開始備份**，彈出密碼輸入框，請輸入密碼授權備份文件，直至文件備份完成。

> ![notes](../common/notes.svg)說明：在備份文件的過程中，請不要拔掉電源或強行關機，以防止資料的遺失或損壞。
...
2. 選擇 **備份/還原** > **還原**，您可以選擇：
   ...
   - 自備份文件還原
     1. 在選擇備份文件參數後，單擊 ![loadfile](../common/loadfile.svg) 選擇要還原的文件。
     2. 單擊 **開始還原**，開始還原系統。
...
- 單擊工作區上方的 ![add](../common/+.svg)。

- 按下鍵盤上的 ![add](../common/+.svg)。
...
- 在桌面，按下鍵盤上的 **AIt** + **Space** 或在視窗標題欄上單擊右鍵，打開視窗快捷操作選單，選擇 **永遠在可見工作區**，或 **移至右方工作區** 或 **移至左方工作區**，將目前視窗移動到指定的工作區。
...
同時按下 **AIt** + **Tab** 並快速釋放，快速切換目前視窗和相鄰程式視窗；

同時按下 **AIt** + **Shift** + **Tab** 並快速釋放，快速反向切換目前視窗和相鄰程式視窗。

#### 快速切換同類型視窗

同時按下 **AIt** + ![~](../common/~.svg) 並快速釋放，快速切換目前同類型視窗；

同時按下 **AIt** + **Shift** + ![~](../common/~.svg) 並快速釋放，快速反向切換目前同類型視窗。

#### 切換所有視窗

按住鍵盤上的 **AIt** 鍵不放，連續按下鍵盤上的 **Tab** 鍵，所有視窗依次向右切換顯示；

按住鍵盤上的 **AIt** + **Shift** 鍵不放，連續按下鍵盤上的 **Tab** 鍵，所有視窗依次向左切換顯示。

#### 切換同類型視窗

按住鍵盤上的 **AIt** 鍵不放，連續按下鍵盤上的 ![~](../common/~.svg) 鍵，目前同類型視窗依次向右切換顯示；

按住鍵盤上的 **AIt** + **Shift** 鍵不放，連續按下鍵盤上的 ![~](../common/~.svg) 鍵，視窗依次向左切換顯示。
