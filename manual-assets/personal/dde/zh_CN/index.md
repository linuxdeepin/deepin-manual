...
| ![uosbrowser-cn](icon/uosbrowser-cn.svg)             | 浏览器 - 点击打开网页。             | ![deepin-appstore](icon/deepin-appstore.svg)             | 应用商店 - 搜索安装应用软件。                       |
...
开启云同步，可自动同步各种系统配置到云端，如网络、声音、鼠标、更新、任务栏、启动器、壁纸、主题、电源等。若想在另一台电脑上使用相同的系统配置，只需登录此Union ID，即可一键同步以上配置到该设备。
...
3. 要还原为之前的屏幕方向，点击鼠标右键退出；要使用当前屏幕方向，请按下组合键 **Ctrl** + **S** 保存。
...
> ![tips](icon/tips.svg)窍门：若要禁用快捷键，请按下键盘上的 ![Backspace](icon/Backspace.svg)。若要取消修改快捷键，按下 **Esc** 键， 或点击下方的”恢复默认”按钮。
...
### 系统更新
当系统存在更新时会在控制中心首页提示，点击 ![update](icon/update.svg) 进入"检查更新"页面，点击 **检查更新**，检查完毕后，即可开始下载和安装更新。您还可以设置自动下载更新。
...
> ![notes](icon/notes.svg)说明：如果系统未激活，授权管理图标 ![uos1](icon/uos1.svg)一直常驻在右下角托盘。
...
1. 进入授权管理界面。
   - 点击右下角托盘上的 ![uos1](icon/uos1.svg)进入授权管理界面。
...
> ![notes](icon/notes.svg)说明：如果系统激活成功，桌面右下角托盘不再显示授权管理图标。

![0|activation_code3](jpg/activation_code3.png)

在授权管理界面，点击 **查看更多增值服务**，可以跳转至服务活动网站。


#### 激活服务器设置

如果当前激活服务器崩溃了，通过授权管理工具，点击![menu](icon/icon_menu.svg) 主菜单中的设置，可以自定义新的激活服务器。
...
3. 点击 **备份**，弹出密码输入框，请输入密码授权备份文件，直至文件备份完成。
...
在开机时，选择系统界面，点击 **UOS Backup & Restore**，进入统信UOS系统还原，用户可以选择“恢复统信UOS出厂设置”读取恢复出厂设置备份，也可以选择“恢复统信UOS历史备份”打开文件目录选取要还原的备份。
...
2. 选择 **备份/还原** > **还原**，您可以选择：
   ...
   - 还原所有设置
      1. 选择是否勾选 **保存用户数据**，点击 **还原**，在弹出的对话框中，点击 **确定**。 
      2. 在弹出的密码输入框中，输入密码授权进行数据的还原。
   - 手动还原
      1. 在选择还原目录参数后，点击![loadfile](icon/loadfile.svg)选择要还原的文件。
      2. 点击 **还原**，开始还原系统。
...
2. 点击 **开发者模式**，点击 **进入开发者模式** 按钮。
   - 若选择在线模式，请先登录Union ID。
      1. 登录Union ID帐号后弹出 **隐私政策** 窗口，勾选  **我已阅读并同意《隐私政策》**并点击 **确定**。
      2. 弹出开发者模式免责声明，仔细查看开发者模式免责声明，了解注意事项后，勾选 **同意并进入开发者模式**，点击 **确定**。待系统下发证书后，**进入开发者模式** 按钮变为 **已进入开发者模式**。
   - 若选择离线模式，根据提示下载证书，待系统导入证书后，即可进入开发者模式。
3. 在弹出的对话框中点击 **现在重启**， 重启系统后开发者模式生效。
