...
> ![notes](icon/notes.svg)说明：只有系统授权后，才可以使用语音听写、语音朗读、翻译及桌面智能助手相关功能。当首次启动桌面智能助手时，需要确定隐私协议，确定同意隐私协议后，可以正常使用桌面智能助手及语音听写、语音朗读、翻译等功能。
...
1. 在控制中心首页，单击 ![display_normal](icon/display_normal.svg)。
2. 单击 **旋转屏幕** ![rotate](icon/rotate.svg) 。
...
1. 在控制中心首页，单击 ![default_applications_normal](icon/default_applications_normal.svg)。
2. 选择文件类型进入默认程序列表。
3. 单击列表下的 **添加默认程序**![add](icon/add.svg)，选择desktop文件（一般在/usr/share/applications)，或特定的二进制文件。
...
1. 在控制中心首页，单击  ![network_normal](icon/network_normal.svg)。
2. 单击 **DSL**，单击 **创建PPPoE连接**![add](icon/add.svg)。
3. 输入宽带名称、帐户、密码。
...
1. 在控制中心首页，单击  ![network_normal](icon/network_normal.svg)。
2. 单击 **VPN**，选择 **添加VPN**![add](icon/add.svg) 或 **导入VPN**![import](icon/import.svg)。
...
1. 在控制中心首页，单击 ![keyboard_normal](icon/keyboard_normal.svg)。
2. 单击 **快捷键**。
3. 单击![add](icon/add.svg)，进入添加自定义快捷键界面。
4. 输入快捷键名称、命令和快捷键。
5. 单击 **添加**。
...
> ![notes](icon/notes.svg)说明：如果系统未激活，授权管理图标 ![authorize1](icon/authorize1.svg)一直常驻在右下角托盘。
...
1. 进入授权管理界面。
   - 点击右下角托盘上的 ![authorize1](icon/authorize1.svg)进入授权管理界面。
...
在系统安装的过程中，仅当选择全盘安装时，会先自动创建恢复分区，再备份根分区和启动分区。

##### 通过控制中心进行备份

用户既可以设置全盘备份，也可以设置系统备份。
...
在开机时，选择系统界面，单击 **UnionTech OS Restore**，进入统信UOS系统还原，用户可以选择“恢复出厂设置”读取安装时的初始化备份，也可以选择“自定义恢复”恢复到某一历史备份时间点。
...
<div class="version-info"><span>文档更新时间: 2020-12-24</span><span> 版本: 2020.04</span></div>