...
2. Locate ![disk-manager](icon/disk-manager.svg) by scrolling the mouse wheel or searching "disk manager" in the Launcher interface and click it to run. An authentication box pops up. Input the UnionTech OS password for verification. 
...
## Disk Partitioning
On the Disk Manager interface, a list and partition information of disk(s) are displayed, including format, capacity and mount point of partitions. You can create, resize, format, mount and unmount a partition.
...
2. A confirmation dialogue pops up. Click **Confirm** to enter the partition operating interface, where you can check the size, name, and file system of this partition, and information of the disk where this partition locates. 
3. In the **Partition Information** area, fill in the name and size of partition, select file system of partition, and click ![add-normal](icon/add-normal.svg) icon. You can create multiple partitions. Each partition and relative name will be displayed on the bar graph of disk. You can delete a partition by clicking the ![edit-delete](icon/edit-delete.svg) icon when you're creating a new partition.
...
5. You can use the newly created partition after it's formatted and then mounted. Please refer to [Format a Partition](#Format a Partition) and [Mount a Partition](#Mount a Partition) for detailed information.

### Resize a Partition

You can resize a partition if its size is too small. 

Notice that you can only resize an unmounted partition.
...
After a partition is hidden, it will not be displayed on the Computer interface. Instead, it will be displayed on the Disk Manager interface. Documents saved in the hidden partition will not be lost, yet they cannot be accessed normally. 

Notice that you can only hide an unmounted partition. Also, system partition cannot be hidden. 

1. On the Disk Manager interface, select a partition and right-click. 
...
1. On the Disk Manager interface, select a hidden partition and right-click. 
...
2. A confirmation dialogue pops up. Click **Unmount** if you confirm that there are no programs running on the disk. 

### Delete a Partition

Once a partition is deleted, you will lose all data in it. Please proceed with caution.

Notice that you can only delete an unmounted partition.
