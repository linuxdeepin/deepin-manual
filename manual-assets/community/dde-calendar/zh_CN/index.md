...
2. 上下滚动鼠标滚轮浏览或通过搜索，找到日历 ![draw](icon/dde_calendar.svg)单击运行。
3. 右键单击 ![draw](icon/dde_calendar.svg) ，您可以：
...
- 在任务栏右键单击 ![draw](icon/dde_calendar.svg) ，选择 **关闭所有** 来退出日历。
- 在日历界面单击 ![icon_menu](icon/icon_menu.svg) ，选择 **退出** 来退出日历。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   ...
   - 日程的时间：全天、开始时间和结束时间。
      ...
      - 当勾选全天时：
         ...
         - 提醒设置：从不、日程发生当天（上午9时）、1 天前、 2 天前和1周前，其中1天前为默认设置。
   
      - 当不勾选全天时：
         - 开始时间、结束时间：既能设置小时和分钟，也能选择日期。
         - 提醒设置：从不、日程开始时、15 分钟前、30 分钟前、1 小时前、1 天前、 2 天前、1周前。
...
1. 在月、周或者日视图中，鼠标右键单击某一个已创建日程的日期。
2. 选择 **编辑**，弹出 **编辑日程** 窗口。
3. 设置编辑日程的相关属性。 
4. 单击 **保存**。 
...
1. 在创建日程或编辑日程时，单击 **重复** 右侧的下拉列表，然后进行选择。
   - **每天**：输入每次重复之间的天数。
   - **工作日**：输入每次重复之间的工作日数。
...
3. 在搜索框中单击![0|close](icon/close.svg)或删除输入的信息，即可清除当前输入的关键字或取消搜索。
...
在日历中，如果选择重复日程的第一次重复，在删除日程时，单击 **全部删除**，可以删除所有的重复日程。

### 删除重复日程的部分重复

在日历中，如果选择重复日程的一次或多次重复，在删除日程时，可以选择单击：
