...
> ![tips](icon/tips.svg): You can also right-click the font file, select **Open with** > ![deepin-font-manager](icon/deepin-font-manager.svg)**Font Manager** to enter the Font Manager interface and click **Install** to install the font.

> ![notes](icon/notes.svg): Font Manager will detect your fonts automatically. If the font has been installed locally, a prompt message will pop up to show that font of the same version has been installed. You can select **Exit** or **Continue** to exit or continue the installation.
...
>
> : Currently only single font export is supported.  Only the latest version of the same font is exported and there is no copy. The system font cannot be exported.
...
When the font is installed successfully, you can view its installation location via Font Manager.
...
The default preview copy is "Don't let your dreams be dreams", displayed in Chinese / English.
...
2.   Set the font size by dragging the control bar horizontally.
   ...
   > ![notes](icon/notes.svg): Slide the control bar to the left to reduce the font size and slide it to the right to enlarge the font size. The font size ranges from 6px to 60px.
...
3.   Click the checked font in the **Activated** list to disable the font.
...
> ![notes](icon/notes.svg): Currently, only single font deletion is supported. Batch deletion of fonts is not supported. System fonts and disabled fonts cannot be deleted.

1. Select a font in User on the Font Manager interface and right-click **Delete**.
2.  Click **Delete** in the pop-up prompt dialogue box.
