...
惬意的悠闲时光，让影院陪您一起度过！影院提供了直观易用的操作界面和丰富齐全的快捷键，依靠键盘您就能完成所有的播放操作，让您彻底摆脱鼠标单击的束缚。您可以使用影院播放多种格式的视频文件。
...
2. 上下滚动鼠标滚轮浏览或通过搜索，找到影院 ![movie_24](icon/movie_24.svg) 单击运行。
...
- 在任务栏右键单击 ![movie_24](icon/movie_24.svg)，选择 **关闭所有** 来退出影院。
...
> ![notes](icon/notes.svg)说明 ：当鼠标置于进度条上时，默认显示预览窗口。若要关闭预览窗口，请在设备中取消勾选 **鼠标悬停进度条时显示预览**。
...
> ![attention](icon/attention.svg) 注意 ：使用影院在线播放视频时，请确保您的电脑是连网状态。
...
- 在影院界面，按下键盘上的 ![Right](icon/Right.svg)键快进视频或![Left](icon/Left.svg)键快退视频。
...
>![notes](icon/notes.svg) 说明 ：在影院窗口上双击鼠标，将在全屏与正常窗口之间切换。
...
> ![notes](icon/notes.svg) 说明：如果您的电脑支持触控屏，在全屏状态下，一指向上滑动视频画面区域增加音量，一指向下滑动视频画面区域减小音量。
...
> ![tips](icon/tips.svg) 窍门： 在播放列表界面，将鼠标指针置于视频文件上，单击 ![关闭图标](icon/close.svg) 删除视频文件。
