...
>![notes](icon/notes.svg) Notes: If no network connection is initiated there will be no alert message, for example, for applications that are not able to connect to network by themselves, there will be no prompts on startup even if **Ask** or **Disable** is selected.
...
<div class="version-info"><span>Update Date: 2022-01-13</span><span> Version: 3.0</span></div>