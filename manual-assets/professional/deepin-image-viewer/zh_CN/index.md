...
2. 上下滚动鼠标滚轮浏览或通过搜索，找到看图 ![viewer](icon/deepin-image-viewer.svg)单击运行。
...
> ![notes](icon/notes.svg)说明：当使用触屏电脑时，在触摸屏上二指张开，放大图片；二指捏合，缩小图片。
...
> ![tips](icon/tips.svg)窍门：单击 **复制** 会同时复制图片路径和内容。
...
3. 图片将会顺时针转旋转90°/逆时针旋转90°。
   ...
   > ![notes](icon/notes.svg)说明：当使用触屏电脑时，在触摸屏上二指放置在图片上并保持一定距离，同时旋转二指来旋转图片。
