...
3. 选择目标位置粘贴。
4. 鼠标移入剪贴板的某一区块，单击该区块右上角的![close](icon/close_normal.svg)按钮，删除当前内容；单击顶部的 **全部清除**，清空剪贴板。

![1|clipboard](jpg/clipboard.png)

## 任务栏
任务栏一般是指位于桌面底部的长条，主要由启动器、应用程序图标、托盘区、系统插件等组成。在任务栏中，您可以打开启动器、显示桌面、进入工作区，对其上的应用程序进行打开、新建、关闭、强制退出等操作，还可以设置输入法，调节音量，连接网络，查看日历，进入关机界面等。
...
| ![dde-file-manager](icon/dde-file-manager.svg) | 文件管理器 - 单击查看磁盘中的文件、文件夹。 | ![ uosbrowser-cn](icon/uosbrowser-cn.svg) | 浏览器 - 单击打开网页。 |
...
| ![dde-calendar](icon/dde-calendar.svg) | 日历 - 查看日期、新建日程。 | ![controlcenter](icon/controlcenter.svg) | 控制中心 - 单击进入系统设置。         |
| ![notification](icon/notification.svg) | 通知中心 - 显示所有系统和应用的通知。 | ![onboard](icon/onboard.svg) | 屏幕键盘 - 单击使用虚拟键盘。 |
...
### 进入关机界面
您可以单击任务栏上的 ![shutdown](icon/system-shutdown.svg) 进入关机界面，也可以在启动器的小窗口模式中单击 ![poweroff_normal](icon/poweroff_normal.svg) 进入关机界面。
...
- 搜索框：输入关键字回车，搜索相应设置。
- 主菜单：单击![menu](icon/icon_menu.svg) 进入主菜单。在主菜单中，您可以查看版本，或退出控制中心。
...
> ![attention](icon/attention.svg): 已登录的帐户无法被删除。
...
3. 要还原为之前的屏幕方向，单击鼠标右键退出；要使用当前屏幕方向，请按下组合键![Ctrl](icon/Ctrl.svg)+![S](icon/S.svg)保存。
...
> ![tips](icon/tips.svg)窍门：在多屏环境下，您还可以按下 **Super** + **P **调出多屏显示模式的OSD。按住 **Super** 不放，按下 **P** 或鼠标单击来进行模式选择。松开按键，确认选择，模式生效。
...
### 个性化设置
在控制中心个性化设置模块可以进行一些通用的个性化设置，包括系统主题、活动用色、字体、窗口透明度，改变桌面和窗口的外观。除此之外还可设置图标主题、光标主题和字体。
...
飞行模式又称航空模式，是断开网络连接的一种方式。在乘坐航空飞机时，为了防止电子设备（如个人电脑）影响或干扰驾驶舱与地面塔台之间的通信系统构成安全隐患。一般会要求在飞机上的乘客将电子设备置于飞行模式。
...
2. 单击 **系统代理**，进入系统代理界面。
 - 单击 **无**，关闭代理服务器功能。
 - 单击 **手动**，输入代理服务器的地址和端口信息，单击 **保存**。
 - 单击 **自动**，输入URL，系统将自动配置代理服务器的信息，单击 **保存**。
...
1. 在控制中心首页，单击 ![nav_notifi_42px](icon/nav_notifi_42px.svg)。
...
> ![notes](icon/notes.svg)说明：当笔记本电脑配备了蓝牙时，您可以直接开启蓝牙开关；当台式电脑没有配备蓝牙时，您可以购买蓝牙适配器，插入到电脑的USB端口中，再使用蓝牙。
...
3. 进入“我的设备”选择已配对连接的蓝牙设备，单击已连接附近的按钮![next](icon/next.svg) 进入详情，单击 **发送文件** 按钮（仅PC/手机/平板的蓝牙设备有该按钮），在弹出的文件管理器窗口中选择要发送的文件，单击 **打开**。
...
1. 在控制中心首页，单击 ![power_normal](icon/power_normal.svg)。
2. 单击 **通用**。
   - 打开 **显示剩余电量及剩余充电时间**，显示剩余电量及剩余可用时间。
...
> ![tips](icon/tips.svg)窍门：若要禁用快捷键，请按下键盘上的 **Backspace** ![Backspace](icon/Backspace.svg) 键。若要取消修改快捷键，按下 **Esc** 键， 或单击下方的”恢复默认”按钮。
...
1. 运行授权管理工具有两种方式：
   - 单击右下角托盘上的 ![uos1](icon/uos1.svg)进入界面。
...
1. 单击右下角托盘上的 ![uos1](icon/uos1.svg)进入界面。

   ![0|probation1](jpg/probation1.png)

2. 单击 **试用期激活**，跳转到激活界面。

3. 单击 **立即激活**，会弹出二次确认弹框，单击 **确定**，弹出试用期激活成功界面，任务栏图标变为![uos2](icon/uos2.svg)。
...
2. 单击 **输入激活码**，输入或复制正确的激活码，单击 **立即激活**。
...
2. 离线激活界面显示二维码、激活码、机器ID及离线激活码输入框。
   ...
   ![0|activate offline1](jpg/activation_offline1.png)
...
如果当前的激活服务器崩溃了，可以打开授权管理工具，单击主菜单中的设置，则可以自定义新的激活服务器。
...
<div class="version-info"><span>文档更新时间: 2022-03-14</span><span> 版本: 2022.02</span></div>