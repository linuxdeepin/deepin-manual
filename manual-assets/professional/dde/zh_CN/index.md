...
1. UOS备份还原工具是帮助在UOS中因人为操作错误、系统软件或应用软件的缺陷、硬件的损毁、电脑病毒、骇客攻击、自然灾害等等诸 多因素造成数据缺失，进行数据或系统还原，从而保障系统和软件的正常运行。
2. 在控制中心界面，选择系统信息，您可以进行系统备份及还原。
3. 系统备份功能有二种模式：
 - 全盘模式：备份全磁盘系统文件和用户产生的文件，全盘备份无法备份在自己的磁盘里，只能备份在其它存储介质。
 - 系统备份：备份根分区和启动分区的文件，系统备份会备份在默认位置，如果磁盘空间不足请提示，可以手动备份在可以存放的磁盘位置。
4. 系统还原：
 - 在系统还原界面，您可以还原所有设置，并勾选保存用户数据，还原后所有设置会将系统盘恢复到您安装系统时的状态，您的数据将保留而用户名和密码会清空，还可以手动还原到指定目录。
