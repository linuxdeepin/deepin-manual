...
2. 点击 **输出**，进入输出设备配置界面，您可以：
   - 在输出设备后面的下拉框中选择输出设备类型。
   - 通过拖曳滑块调节输出音量和左/右声道平衡。
   - 打开 **音量增强**，音量的可调节区间由0~100% 转变为0~150%。
> ![tips](icon/tips.svg)窍门：若关闭输出设备音量设置，系统全部静音，您将无法听到系统音效和声音。


#### 输入设备

1. 在控制中心首页，点击 ![sound_normal](icon/sound_normal.svg)。
2. 点击 **输入**，进入输入设备配置界面，您可以：
   - 通过拖曳滑块调节输入音量。
...
3. 进入“我的设备”选择已配对连接的蓝牙设备，点击进入详情，点击 **发送文件** 按钮（pc/手机/平板蓝牙设备类型才具有该按钮），弹出选择发送文件对话框。
4. 点击 **选择文件**，在弹出的文件管理器窗口中选择要发送的文件，并点击 **发送**，发送传输文件请求。
...
- 如果接收端为如下情况，发送端部分文件传输失败，会显示在传输失败列表。
   ...
   - 接收端即不点击接收也不点击拒绝，默认1分钟后则结束传输，显示传输失败。
...
<div class="version-info"><span>文档更新时间: 2020-09-09</span><span> 版本: 2020.03</span></div>
