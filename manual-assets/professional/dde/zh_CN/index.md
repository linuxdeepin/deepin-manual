...
1. 在控制中心首页，单击 ![account_normal](../common/account_normal.svg)。
2. 单击当前帐户。
3. 打开 **无密码登录** 开关，在弹出的授权对话框中输入当前帐户密码并确定后，开启后可以不用输入密码登录系统。
...
> ![notes](../common/notes.svg) 说明：多屏扩展模式时，任务栏右键菜单显示 **多屏显示设置**，多屏显示设置包含 **仅主屏显示**、**跟随鼠标位置显示** 两个互斥选项。
...
多屏屏幕亮度、色温、屏幕缩放、分辨率、刷新率、方向的设置方式与单屏设置方式相同，详细操作方法请参考 [单屏设置](#单屏设置)。
