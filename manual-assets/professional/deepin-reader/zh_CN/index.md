...
2. 上下滚动鼠标滚轮浏览或通过搜索，找到文档查看器 ![deepin-reader-24](icon/deepin-reader-24.svg)单击运行。
...
- 单击 ![icon_menu](icon/icon_menu.svg) > **保存** 或使用 **Ctrl + S** 组合键保存文件。
- 单击 ![icon_menu](icon/icon_menu.svg) > **另存为** 或使用 **Ctrl + Shift + S**组合键另存文件。
...
- 当标签页达两个或两个以上时，可单击 **+** 按钮添加新标签页。

切换/调整标签页

- 同一窗口内拖拽标签页调整排序。
- 当标签页过多时会显示 ![previous](icon/previous.svg) 或 ![next](icon/next.svg) 按钮，单击可左右移动。
- 滚动鼠标切换标签页。 
...
- 复制注释
   -  在页面上，右键单击已完成注释的文本内容，在右键菜单中选择 **复制**，可以复制该条注释的全部内容。
   -  在页面上，右键单击注释图标，选择 **复制**，可以复制该条注释的全部内容。
...
- 删除注释
   - 右键单击已完成注释的高亮文本内容，选择 **取消高亮**，可以删除该条注释内容。
   - 右键单击注释图标，选择 **删除注释**，可以删除该条注释内容。
...
5.  当搜索到匹配信息后，单击搜索窗口的 ![search](icon/previous-up.svg) 或 ![search](icon/next-down.svg) 可逐个查找上一个或下一个匹配项。
