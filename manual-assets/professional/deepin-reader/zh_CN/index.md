...
> ![tips](icon/tips.svg)窍门：您还可以使用![Ctrl](icon/Ctrl.svg) + ![Shift](icon/Shift.svg) + ![S](icon/S.svg) 或点击![icon_menu](icon/icon_menu.svg) > **另存为**，另存文件。
...
> ![notes](icon/notes.svg)说明：软件记住用户上次选择的高亮显示颜色，当下次用户再进行文本高亮显示操作时，默认使用用户上次选择的高亮颜色。用户可以根据喜好切换可供选择的高亮颜色。
