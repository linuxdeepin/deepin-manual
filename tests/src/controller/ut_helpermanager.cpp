...
bool ut_dbussend = false;

QDBusMessage ut_helpcall(const QDBusMessage &message, QDBus::CallMode mode = QDBus::Block,
                         int timeout = -1)
{
    ut_dbussend = true;
    QDBusMessage busmsg;
...
QString ut_helperManager::stub_writableLocation(QStandardPaths::StandardLocation type)
{
    return "/tmp";
}
...
TEST_F(ut_helperManager, handlePriority)
{
    ...
    QStringList retList = m_hm->handlePriority(addList);
    ASSERT_EQ(retList[0], "/usr/share/deepin-manual/manual-assets/application/deepin-terminal/terminal/zh_CN/voice-note.md");
}
...
TEST_F(ut_helperManager, onRecvParseMsg)
{
    ...
    QString sql = "select count(1) from search where appname ='deepin-voice-note' and content like 'Voice Notes is a simple memo software with texts%' and anchorid='h1' and lang = 'en_US'";
    ...
    if (!db.open()) {
        qCritical() << "Failed to open search db:" << databasePath << db.lastError().text()
                    << db.lastError().nativeErrorCode() << db.lastError().type()
    ...
    sql = "select count(1) from search where appname ='deepin-voice-note' and content like '语音记事本是一款设计简洁%' and anchorid='h1' and lang = 'zh_CN'";
    query.exec(sql);
    query.next();
    ...
    sql = "select count(1) from search where appname ='deepin-voice-note' and content like '语音记事本是一款设计简洁%' and anchorid='h1' and lang = 'ec_AS'";
    query.exec(sql);
    query.next();
    icount = query.value(0).toInt();
    query.next();
...
{
    Stub st;
    st.set(ADDR(QStandardPaths, writableLocation), ADDR(ut_helperManager, stub_writableLocation));
    st.set((QDBusMessage(QDBusConnection::*)(const QDBusMessage &, QDBus::CallMode,
                                             int) const)ADDR(QDBusConnection, call),
           ut_helpcall);

    QString dbdir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation).append("/.local/share/deepin/deepin-manual");
    QString databasePath = dbdir.append("/search.db");

    m_hm->deleteTList << databasePath;
    m_hm->onTimerOut();
    ASSERT_TRUE(ut_dbussend);
}
