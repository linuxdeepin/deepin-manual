...
在文件管理器中点击搜索按钮，或使用 ![Ctrl](icon/Ctrl.svg) + ![F](icon/F.svg) 快捷键进入搜索状态，或在地址栏中输入关键词回车，搜索相关文件。
...
1. 输入局域网用户的共享地址，如：smb://xx.x.xx.xxx（一般为IP地址），按下键盘上的 ![Enter](icon/Enter.svg) 键。
2. 输入用户名密码或者匿名访问。
