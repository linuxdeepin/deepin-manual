...
> ![tips](icon/tips.svg)：在列表视图中，把光标置于两列之间的分隔线上， 拖动它来改变列的宽度。
...
深度文件管理器支持空格键预览功能，选中文件并按下键盘上的空格键就可以快速预览文件。预览窗口可以查看图片分辨率、文档大小、文本内容等，还支持音频和视频播放。
...
> ![tips](icon/tips.svg)：可点击导航栏中磁盘右边的![unmount](icon/unmount_normal.svg)卸载磁盘。
