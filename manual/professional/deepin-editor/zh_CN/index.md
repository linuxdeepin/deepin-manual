...
编辑器是一个简单的文本编辑工具。您可以用它书写简单的文本文档，也可以使用它的高级特性，让它成为一个代码编辑工具，支持代码语法高亮。
...
点击桌面底部的 ![deepin-launcher](icon/deepin-launcher.svg) ，进入启动器界面。搜索或浏览找到编辑器 ![deepin-editor](icon/deepin-editor.svg)，点击打开。
...
- 在编辑器界面，点击  ![close_icon](icon/close_icon.svg) ，退出编辑器。
- 在编辑器界面，点击主菜单 ![icon_menu](icon/icon_menu.svg) > **退出** 来退出编辑器。
- 在任务栏右键单击 ![deepin-editor](icon/deepin-editor.svg) ，选择 **关闭所有** 来退出编辑器。
...
- 直接拖动文件到编辑器界面或其图标上。
- 右键文件，选择用编辑器打开。选择编辑器为默认打开程序后，可直接双击打开。
- 在编辑器界面，点击![icon_menu](icon/icon_menu.svg) > **打开文件**，选择文件。
- 在编辑器界面，使用![Ctrl](icon/Ctrl.svg) + ![O](icon/O.svg)组合键，选择文件打开。
...
> ![notes](icon/notes.svg)：如果您的文件中发生了变化而没有保存，编辑器会在关闭前提示您保存更改。
...
在编辑器中，可以拖动同一个窗口内的标签页重新排序，也可以移出标签页创建一个新的窗口，或者从一个窗口移到另外一个窗口中。
...
在控制中心>辅助功能中开启文本翻译，根据需要将语言设置为英译中或者中译英。在编辑器界面选中一段文本后点击右键，选择 **文本翻译**，可将该段文本翻译成目标语言。
![text translation](jpg/text translation.png)

## 主菜单

在主菜单中，您可以 [新建窗口](#新建文件)、[新建标签页](#新建文件)、查找替换内容、保存文件、切换窗口主题、查看帮助手册，了解编辑器的更多信息。
...
1. 在编辑器界面，点击![icon_menu](icon/icon_menu.svg)。
2. 点击 **主题**，选择一个主题颜色。
