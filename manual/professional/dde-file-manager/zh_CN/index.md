...
它沿用了传统文件管理器的经典功能和布局，并在此基础上简化了用户操作，增加了很多特色功能。一目了然的导航栏、智能识别的搜索框、多样化的视图及排序让您管理起来得心应手，经典和超越，只为给您更好的体验！
...
| 3    | 图标/列表视图 | 点击 ![icon_view](jpg/icon_views.png) , ![list_view](jpg/list_views.png) 图标，以图标或列表形式查看文件（夹）。 |
| 4    | 信息栏        | 点击 ![fileinfo](jpg/fileinfo.png)， 查看文件（夹）的基本信息和标记。 |
...
1. 在文件管理器界面上，右键单击文件。
2. 选择 **重命名**。
3. 输入文件名称，按 **Enter** 键或者鼠标点击界面空白区域。
...
点击界面上的 ![icon_view](jpg/icon_views.png) 、![list_view](jpg/list_views.png)  图标来切换图标视图和列表视图。
...
> ![tips](icon/tips.svg)：使用  **Ctrl**+**1**  和  **Ctrl**+ **2**   快捷键，切换图标视图和列表视图。
...
> ![notes](icon/notes.svg)：查看多个文件属性会显示文件总大小和数量；查看快捷方式的属性将会额外显示源文件地址。您还可以使用 **Ctrl** + **I** 组合键查看文件属性。
...
在文件管理器中点击搜索按钮，或使用  **Ctrl** + **F** 快捷键进入搜索状态，或在地址栏中输入关键词回车，搜索相关文件。
  - 需要指定目录搜索时，请先进入该目录，然后再进行搜索。

> ![notes](icon/notes.svg)：在文件管理器的 **设置 **中，默认勾选了“自动索引内置磁盘”，您可以选择勾选“连接电脑后索引外部存储设备”，加快在外部设备的搜索速度。

#### 高级搜索
1. 在搜索状态下，输入关键词后按回车，点击搜索栏右侧的 ![filter](icon/filter.svg)进入高级搜索。
2. 选择搜索范围、文件类型、文件大小和修改时间进行更精准的搜索。
...
> ![tips](icon/tips.svg)：文件管理器窗口只有一个标签时，标签栏是隐藏的，您还可以使用快捷键  **Ctrl** + **T** 来新建标签。
...
##### 通过右侧信息栏添加标记
选择文件(夹)后，点击 ![fileinfo](jpg/fileinfo.png)，在右侧信息栏中也可以添加标记。

![info tag](jpg/info tag.png)

通过上述其中一个步骤完成标记后可在左侧导航栏最底部查看标记，点击该标记可迅速找到该文件。

![1|show tag](jpg/show tag.png)
...
### 权限管理
选择对应文件/文件夹，单击右键，选择属性，点击 **权限管理**， 为所有者，群组，和其他用户设置文件权限。
...
1. 在文件管理器界面，点击主菜单![icon_menu](icon/icon_menu.svg) ，选择**连接到服务器**。

![1|connect to server](jpg/connect to server.png)
2. 输入局域网用户的共享地址，如：smb://xx.x.xx.xxx（一般为IP地址），按下键盘上的 **Enter** 键或点击**连接**。
...
> ![notes](icon/notes.svg)：如果磁盘或磁盘中的文件夹被加密了，请输入密码后查看文件。
...
> ![tips](icon/tips.svg)：如果要弹出光盘，请选择 **弹出** 来移除光盘。点击导航栏中磁盘右边的![unmount](icon/unmount_normal.svg)同样可以弹出磁盘或光盘。
