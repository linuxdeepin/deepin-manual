...
![1|label](jpg/label.png)

- 支持在同一窗口内拖拽调整标签页排序。

- 支持点击![previous](icon/previous.svg)![next](icon/next.svg)按钮，可左右移动标签（当标签页显示区域达到上线后箭头按钮才会出现。

- 首个标签页，左移按钮置灰，选择最后一个标签页，右移按钮置灰。

- 将光标放置标签页上滚动鼠标支持自动切换标签页。 
...
- 添加注释
   ...
   1.  使用选择工具选择需要注释的文本内容，点击右键，选择 **添加注释**，完成注释后该文本内容显示高亮。
   2.  将光标移动到需要注释地方，点击鼠标左键选择**添加注释**，插入注释，该地方会显示一个注释图标。
   3.  点击左侧缩略图栏下方的注释图标，点击**添加注释**按钮，点击当前页面对所需要注释的地方加以注释。
   ![1|add comment](jpg/add comment.png)
   4.  点击注释窗口 右上角的“ **x** "关闭按钮或其他空白区域，自动保存输入内容。
...
- 复制注释
   ...
   -  点击![menu](icon/thumbnail_normal_light.svg)，在左侧最底部选择注释目录![view](icon/comments_normal.svg)，选定一个注释，右键单击选择 **复制**，可以复制这条注释的全部内容。
...
- 转至指定页面：
  - 输入页码数并按回车键。
  - 如果您需要回到前面访问的页面，在缩略图下点击下方 ![previous](icon/previous.svg)或 ![next](icon/next.svg)。
