...
2. 通过浏览找到 ![deepin-log-viewer](icon/deepin-log-viewer.svg) 点击运行。
...
- 按级别筛选：按照事件的级别高低进行筛选的方式。
  ...
  选项包括：紧急、严重警告、严重、错误、警告、注意、信息、调试，默认选项为信息；
...
3. 选中某条日志后，列表下方会显示该日志的详细信息，包括日志进程、主机名、进程号、时间和详细信息。
...
3. 选中某条日志后，列表下方会显示该日志的详细信息，包括进程、主机名、时间、级别和详细信息。

> ![0|applog](jpg/applog.png)
>
> ![notes](icon/notes.svg)：在查看日志的过程中，如果有弹框出现 **查看日志需要授权**，输入登录密码即可继续查看。
