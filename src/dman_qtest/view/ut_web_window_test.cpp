...
TEST_F(ut_web_window_test, eventFilter)
{
    ...
    web.initWeb();
    web.setFocus();
    QTest::keyPress(web.search_edit_, Qt::Key_1);
    qDebug() << "searchText---> :  " << web.search_edit_->text();
//    ASSERT_EQ(web.search_edit_->text(), "1");
}
...
TEST_F(ut_web_window_test, hasSearchEditRect)
{
    WebWindow web;
    QRect rect;
    rect.setWidth(web.search_edit_->width());
    rect.setHeight(web.search_edit_->height());
    ASSERT_EQ(web.hasSearchEditRect().width(), rect.width());
    ASSERT_EQ(web.hasSearchEditRect().height(), rect.height());
}
